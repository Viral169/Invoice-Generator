...
const CreateInvoice = () => {
  ...
  const updateLine = (index, field, value) => {
    ...
    if (field === 'quantity' || field === 'unitPrice') {
      updated[index].lineTotal = Number(updated[index].quantity) * Number(updated[index].unitPrice);
    }
    setLines(updated);
  };

  const addLine = () => {
    setLines([...lines, { description: '', quantity: 1, unitPrice: 0, lineTotal: 0 }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (lines.some(l => !l.description || l.quantity <= 0 || l.unitPrice <= 0)) {
      setError("Please fill all line items properly (quantity and unit price must be greater than 0)");
      return;
    }
    if (total <= 0) {
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Invoice number will be auto-generated</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-8">
              ...
              <div className="hidden md:grid grid-cols-12 gap-3 px-4 mb-2">
                <span className="col-span-5 text-[10px] font-semibold text-slate-400 uppercase">Description</span>
                <span className="col-span-2 text-[10px] font-semibold text-slate-400 uppercase">Qty</span>
                <span className="col-span-2 text-[10px] font-semibold text-slate-400 uppercase">Unit Price (₹)</span>
                <span className="col-span-2 text-[10px] font-semibold text-slate-400 uppercase text-right">Line Total</span>
                <span className="col-span-1"></span>
              </div>

              <div className="space-y-2">
                {lines.map((line, i) => (
                  <div key={i} className="grid grid-cols-12 gap-3 items-center p-3 bg-slate-50 rounded-xl">
                    ...
                    <div className="col-span-3 md:col-span-2 text-right">
                      <p className="py-2 text-sm font-bold text-slate-700">₹{(line.lineTotal || 0).toLocaleString()}</p>
                    </div>
